...
describe("Schema migration", () => {
  it("migrates v1 paths/links to v4 schema while preserving compatibility", () => {
    ...
    // v7: provenance columns exist
    expect(memoryCols.some((c) => c.name === "source_session")).toBe(true);
    expect(memoryCols.some((c) => c.name === "source_context")).toBe(true);
    expect(memoryCols.some((c) => c.name === "observed_at")).toBe(true);
